...
namespace BeSpoke.Models
{
	...
	public class DTParameterModel
	{
		...
		//public void SetQueryableSort<T>(ref IQueryable<T> query) where T : class
		//{
		//	foreach (var sort in Order)
		//	{
		//		string colName = Columns.ElementAt(sort.Column).Data;
		//		query = sort.Dir == "desc" ?
		//			query.OrderByDescending(colName)
		//			: query.OrderBy(colName);
		//	}
		//}
			...
			get
			{
				return Order.Any() ?
						String.Join(
							", ",
							Order.Select(s =>
								Columns.ElementAt(s.Column).Data
								+ " " + s.Dir
							)
						)
						: null;
		...
		public ItemPager ToItemPager(string defaultOrderByClause)
		{
			return new ItemPager
			{
				Length = Length,
				OrderByClause = OrderByClause ?? defaultOrderByClause,
				Start = Start
			};
		}
	}
	...
	public class DTModelBinder : DefaultModelBinder
	{
		public override object BindModel(ControllerContext controllerContext, ModelBindingContext bindingContext)
		{
			...
			int draw = int.TryParse(request["draw"], out parsed) ? parsed : 0;
			int start = int.TryParse(request["start"], out parsed) ? parsed : 0;
			int length = int.TryParse(request["length"], out parsed) ? parsed : 0;
